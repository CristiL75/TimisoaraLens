"""
Add auto-scraped landmarks (individual sections) to Qdrant.
Skips the long overview; uses data/timisoara_landmarks_chunks.json.
"""
...
CHUNKS_FILE = Path(__file__).parent.parent / "data" / "timisoara_landmarks_chunks.json"

if not QDRANT_URL or not QDRANT_API_KEY:
    raise ValueError("QDRANT_URL and QDRANT_API_KEY not set")


EXCLUDE_HEADINGS = {"Heritage Landmarks Overview"}
...
def add_to_qdrant():
    ...
    # Filter out overview
    chunks = [c for c in chunks if c.get("heading") not in EXCLUDE_HEADINGS]
    ...
    for i, chunk in enumerate(chunks):
        ...
        payload = {
            ...
            "category": chunk.get("category", "Landmarks"),
            "source": chunk.get("source", "Visit Timișoara - Landmarks"),
            "url": chunk.get("url", "https://visit-timisoara.com/timisoara-landmarks/"),
