...
def get_fallback_message(detected_lang: str) -> str:
    """Get 'I don't know' message in user's language."""
    fallback_messages = {
        "en": "I don't have information about that in my knowledge base. Please try a different question.",
        "ro": "Nu am informații despre asta în baza de cunoștințe. Te rog, pune o altă întrebare.",
        "de": "Ich habe keine Informationen dazu in meiner Wissensbasis. Bitte versuchen Sie eine andere Frage.",
        "fr": "Je n'ai pas d'informations à ce sujet dans ma base de connaissances. Veuillez poser une autre question.",
        "es": "No tengo información sobre eso en mi base de conocimiento. Por favor, haz una pregunta diferente.",
        "it": "Non ho informazioni su questo nella mia base di conoscenza. Prova un'altra domanda.",
        "hu": "Nincs információm erről az ismeretbázisban. Kérlek, egy másik kérdést tegyél fel.",
    }
...
async def rag_query(request: RAGQueryRequest):
    ...
    try:
        ...
        # Search in Qdrant with lower threshold for better recall
        logger.info(f"Searching top {request.top_k} documents...")
        search_result = qdrant_client.query_points(
            collection_name=COLLECTION_NAME,
            query=query_embedding.tolist(),
            limit=request.top_k,
            score_threshold=0.3,  # Lowered from 0.5 to catch more relevant docs
        ).points
        ...
        for i, result in enumerate(search_result, 1):
            ...
            context_text += f"[Source {i}] {payload.get('heading', 'N/A')}\n"
        ...
        # If no relevant documents found, return localized "I don't know" message
        if not search_result:
            return RAGQueryResponse(
                query=request.query,
                answer=get_fallback_message(detected_lang),
                sources=[],
            )
        ...
        system_prompt = (
            f"You are a friendly local from Timișoara helping visitors. "
            f"Answer naturally like you're chatting with a friend. "
            f"{answer_language} "
            f"Keep it brief and helpful. "
            f"NEVER use phrases like: 'based on', 'according to', 'the context shows', 'the information provided'. "
        ...
        user_prompt = f"""{conversation_context}Context:
{context_text}
